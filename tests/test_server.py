...
def mock_client(sample_company, sample_filing, sample_statement_data):
    """Create a mock EdinetClient with default returns."""
    client = MagicMock()
    client.search_companies.return_value = [sample_company]
    client.get_company.return_value = sample_company
    client.get_filings.return_value = [sample_filing]
    client.get_financial_statements.return_value = FinancialStatement(
...
class TestSearchCompanies:
    ...
    async def test_search_limits_to_20(self, mock_client, sample_company):
        mock_client.search_companies.return_value = [sample_company] * 30
        with patch("edinet_mcp.server._get_client", return_value=mock_client):
            result = await _search_companies("トヨタ")
...
class TestGetFilings:
    ...
    async def test_get_filings_limits_to_50(self, mock_client, sample_filing):
        mock_client.get_filings.return_value = [sample_filing] * 60
        with patch("edinet_mcp.server._get_client", return_value=mock_client):
            result = await _get_filings("E02144", "2024-01-01")
